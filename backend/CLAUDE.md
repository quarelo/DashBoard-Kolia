...
- **All DDL lives in Alembic, in Python.** `backend/migrations/versions/` holds `0001_initial_schema` and `0002_meeting_versions`; the IA has its own revisions (`0001`–`0009`) and its own version table, in schema `ai`. There is no `.sql` file anywhere in the repo — `infra/postgres/init.sql` was deleted too, since `ia/migrations/env.py` and revision `0001` both issue `CREATE EXTENSION IF NOT EXISTS vector` (verified when it was removed: migrating a virgin database with no init hook produced the extension at 0.8.6 and every `ai` table).
