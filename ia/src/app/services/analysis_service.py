...
def analyze_meeting(db: Session, payload: AnalyzeRequest) -> MeetingAnalysis:
    ...
    try:
        ...
        for index, content in enumerate(chunks, start=1):
            ...
            db.add(MeetingChunk(analysis_id=analysis.id, external_meeting_id=payload.meeting_id, external_user_id=payload.user_id, chunk_index=index, token_count=count_tokens(content), content=content, clean_content=clean_content, chunk_summary=summary, embedding=generate_embedding(clean_content)))
